...
OUT_DIR="${5:?out_dir}"
LAYER_IDX="${6:?layer_idx}"
METRIC_NAME="${7:?metric_name}"
ACTION_SUBSET="${8:?action_subset}"
REDUCE_MODE="${9:?reduce_mode}"
...
export ROBOSUITE_LOG_PATH="${REPO_ROOT}/logs/robosuite_fast_gpu${GPU}_${METRIC_NAME}_layer${LAYER_IDX}_${REDUCE_MODE}.log"
...
for idx in $(seq "${RUN_START}" "${RUN_END}"); do
  seed=$((SEED_BASE + idx))
  echo "[GPU${GPU}] start run ${idx} seed=${seed}"
  output=$(
    "${PYTHON}" eval/libero/run.py \
      ...
      --decoder-metric-select-layer "${LAYER_IDX}" \
      --decoder-metric-select-name "${METRIC_NAME}" \
      --decoder-metric-select-action-subset "${ACTION_SUBSET}" \
      --decoder-metric-select-reduce "${REDUCE_MODE}" \
      --diagnostics-mode decoder_hidden \
      --decoder-capture-block-indices "${LAYER_IDX}" \
...
PY
  ...
  echo "[GPU${GPU}] done run ${idx} seed=${seed} success=${success}"
done
