...
NUM_RUNS="${1:-70}"
OUT_ROOT="${2:-/home/motovilovil/Robotics/robotics_project/mimic-video/eval_outputs/libero_spatial/decoder_metric_select/task0_init9_${NUM_RUNS}rollouts_norm_std_layer23_max}"
...
while read -r run_start run_end; do
  ...
  if [[ "${run_start}" -eq 0 ]]; then
    echo "[smart_select_max] skip GPU ${gpu}"
    idx=$((idx + 1))
    continue
  fi

  log_path="${LOG_DIR}/gpu${gpu}.log"
  echo "[smart_select_max] GPU ${gpu} -> runs ${run_start}-${run_end}"
  (
