...
PORT="${PORT:-5595}"
SESSION="${SESSION:-groot_n17_libero_pro_spatial_consensus_gpu3}"
DATE_TAG="${DATE_TAG:-$(date +%Y%m%d_%H%M%S)}"
OUT_ROOT="${OUT_ROOT:-${REPO_ROOT}/eval_outputs/libero_pro/groot_n17_spatial_consensus_seeds_1_999_998_gpu3_${DATE_TAG}}"
...
for attempt in {1..300}; do
  grep -q "GR00T consensus-only ready" "${OUT_ROOT}/logs/server_gpu${GPU}.log" && break
...
for split in lan object swap task; do
  for task_index in {0..9}; do
    ...
    printf '[%s] split=%s task=%s candidate_seeds=${CANDIDATE_SEEDS} gpu=${GPU}\n' \
    ...
    "${CLIENT_PY}" scripts/eval/collect_groot_n17_libero_pro_spatial_consensus.py \
...
EOF
chmod +x "${RUNNER}"
tmux new-session -d -s "${SESSION}" -n libero_pro_spatial_consensus "bash ${RUNNER}"
printf 'session=%s\noutput=%s\ncandidate_seeds=%s\n' \
