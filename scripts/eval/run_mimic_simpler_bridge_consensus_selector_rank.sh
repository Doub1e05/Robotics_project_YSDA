#!/usr/bin/env bash
# Evaluate two Bridge tasks per GPU with one fixed-candidate consensus selector.
set -euo pipefail
RANK="${1:?usage: $0 <rank 0..1> <gpu>}"
GPU="${2:?usage: $0 <rank 0..1> <gpu> <selector>}"
SELECTOR="${3:?usage: $0 <rank 0..1> <gpu> <selector>}"
case "${SELECTOR}" in
  action_consensus) SELECTOR_ARGS=(--vam-consensus-medoid-only --vam-consensus-num-candidates 3) ;;
  encoder_latent_medoid) SELECTOR_ARGS=(--vam-latent-medoid-strategy encoder --vam-consensus-num-candidates 3) ;;
  encoder_latent_medoid_robust) SELECTOR_ARGS=(--vam-latent-medoid-strategy encoder_robust --vam-consensus-num-candidates 3) ;;
  decoder_action_token_medoid) SELECTOR_ARGS=(--vam-latent-medoid-strategy decoder_action_tokens --vam-consensus-num-candidates 3) ;;
...
run_task() {
  ...
  "${PYTHON}" "${REPO_ROOT}/eval/bridge/SimplerEnv/simpler_env/main_inference.py" \
    --ckpt-path "ftcosmos_stop0_${SELECTOR}_k3_fixedseeds_2_996_997" --robot "${robot}" --policy-setup widowx_bridge \
