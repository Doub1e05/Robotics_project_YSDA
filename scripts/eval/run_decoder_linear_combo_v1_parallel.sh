...
NUM_RUNS="${1:-30}"
OUT_ROOT="${2:-/home/motovilovil/Robotics/robotics_project/mimic-video/eval_outputs/libero_spatial/decoder_metric_select/task0_init9_${NUM_RUNS}rollouts_linear_combo_v1_layer23}"
...
SEED_BASE=71000
LAYER_IDX=23
METRIC_NAME="decoder_linear_combo_v1"
...
while read -r run_start run_end; do
  ...
  if [[ "${run_start}" -eq 0 ]]; then
    echo "[linear_combo_v1] skip GPU ${gpu}"
    idx=$((idx + 1))
    continue
  fi

  log_path="${LOG_DIR}/gpu${gpu}.log"
  echo "[linear_combo_v1] GPU ${gpu} -> runs ${run_start}-${run_end}"
  (
