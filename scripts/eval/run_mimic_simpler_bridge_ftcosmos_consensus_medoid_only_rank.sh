#!/usr/bin/env bash
# Evaluate one of four disjoint 24-rollout slices of the fixed Bridge ftcosmos stop=0 Consensus_medoid_only protocol.
set -euo pipefail
...
run_task() {
  ...
  "${PYTHON}" "${REPO_ROOT}/eval/bridge/SimplerEnv/simpler_env/main_inference.py" \
    --ckpt-path "ftcosmos_stop0_consensus_medoid_only_k3_fixedseeds012" \
    ...
    --vam-consensus-medoid-only --vam-consensus-num-candidates 3 \
