#!/usr/bin/env bash
# Consensus-medoid with ONLY the consensus term (C[i]); no continuity/smoothness/gripper penalties.
set -euo pipefail
...
OUT_DIR="${OUT_DIR:-${REPO_ROOT}/eval_outputs/libero_spatial/consensus_medoid/task0_init9_consensus_only_${NUM_ROLLOUTS}rollouts_gpu${GPU}_${DATE_TAG}}"
...
export ROBOSUITE_LOG_PATH="${REPO_ROOT}/logs/robosuite_task0_init9_consensus_only_gpu${GPU}.log"
...
echo "=== LIBERO-Spatial task 0 / init 9 · Consensus-only (cost = C[i]) ==="
echo "GPU=${GPU} rollouts=${NUM_ROLLOUTS} seeds=${SEED_BASE}..$((SEED_BASE + NUM_ROLLOUTS - 1))"
echo "λ_c=λ_s=λ_g=0 (no continuity / smoothness / gripper-switch penalties)"
echo "Output: ${OUT_DIR}"
...
"${PYTHON}" "${REPO_ROOT}/scripts/eval/run_boundary_multi_seed.py" \
  ...
  --progress-label "task0 init9 consensus-only ${NUM_ROLLOUTS}x" \
