...
PORT="${PORT:-5596}"
SESSION="${SESSION:-groot_n17_intact_object_ood_decoder_gpu1}"
DATE_TAG="${DATE_TAG:-$(date +%Y%m%d_%H%M%S)}"
OUT_ROOT="${OUT_ROOT:-${REPO_ROOT}/eval_outputs/intact_simpler/groot_n17_object_ood_8tasks_decoder_action_token_medoid_seeds_1_999_998_gpu1_${DATE_TAG}}"
...
CUDA_VISIBLE_DEVICES="${GPU}" "${SERVER_PY}" \
  scripts/eval/groot_n17_decoder_action_token_medoid_server.py \
  --model-path "${MODEL_PATH}" --port "${PORT}" --candidate-seeds "${CANDIDATE_SEEDS}" \
  > "${OUT_ROOT}/logs/server_gpu${GPU}.log" 2>&1 &
server_pid=\$!
for attempt in {1..300}; do
  grep -q "GR00T decoder-action-token-medoid ready" \
    "${OUT_ROOT}/logs/server_gpu${GPU}.log" && break
...
grep -q "GR00T decoder-action-token-medoid ready" \
  "${OUT_ROOT}/logs/server_gpu${GPU}.log"
...
for task in "\${TASKS[@]}"; do
  ...
  CUDA_VISIBLE_DEVICES="${GPU}" "${CLIENT_PY}" \
    scripts/eval/groot_n17_intact_object_ood_client.py \
    --task-key "\${task}" --host 127.0.0.1 --port "${PORT}" \
    --output "${OUT_ROOT}/results/\${task}.json" \
    --episodes 24 --environment-seed 0 --max-episode-steps 300 --execution-horizon 4 \
    --video-dir "${OUT_ROOT}/videos/\${task}" \
    > "${OUT_ROOT}/logs/\${task}.log" 2>&1
  "${CLIENT_PY}" scripts/eval/collect_groot_n17_intact_object_ood_decoder.py \
...
EOF
chmod +x "${RUNNER}"
tmux new-session -d -s "${SESSION}" -n object_ood_decoder "bash ${RUNNER}"
printf 'session=%s\noutput=%s\ncandidate_seeds=%s\n' \
