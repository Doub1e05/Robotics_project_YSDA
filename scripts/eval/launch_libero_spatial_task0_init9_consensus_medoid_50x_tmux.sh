...
SESSION="${SESSION:-mimic_task0_init9_medoid_50}"
GPU="${GPU:-1}"
DATE_TAG="${DATE_TAG:-$(TZ=Asia/Almaty date +%Y%m%d)}"
RUNNER="${REPO_ROOT}/scripts/eval/run_libero_spatial_task0_init9_consensus_medoid_50x.sh"
OUT_DIR="${OUT_DIR:-${REPO_ROOT}/eval_outputs/libero_spatial/consensus_medoid/task0_init9_50rollouts_gpu${GPU}_${DATE_TAG}}"
