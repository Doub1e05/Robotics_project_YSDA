...
GPU="${GPU:-2}"
PORT="${PORT:-5572}"
SESSION="${SESSION:-groot_n17_decoder_action_token_medoid_gpu2}"
DATE_TAG="${DATE_TAG:-$(date +%Y%m%d_%H%M%S)}"
OUT_ROOT="${OUT_ROOT:-${REPO_ROOT}/eval_outputs/simpler_bridge/groot_n17_decoder_action_token_medoid_seeds_1_999_998_${DATE_TAG}}"
for path in "${SERVER_PY}" "${CLIENT_PY}" "${MODEL_PATH}/model.safetensors.index.json"; do
  [[ -e "${path}" ]] || { echo "Missing required path: ${path}" >&2; exit 1; }
done
"${CLIENT_PY}" -c 'import msgpack_numpy' || { echo "Missing msgpack-numpy in SIMPLER env." >&2; exit 1; }
...
CUDA_VISIBLE_DEVICES="${GPU}" "${SERVER_PY}" scripts/eval/groot_n17_decoder_action_token_medoid_server.py \\
  --model-path "${MODEL_PATH}" --port "${PORT}" --candidate-seeds 1,999,998 \\
...
for task in simpler_env_widowx/widowx_carrot_on_plate simpler_env_widowx/widowx_spoon_on_towel simpler_env_widowx/widowx_stack_cube simpler_env_widowx/widowx_put_eggplant_in_basket; do
  name=\${task##*/}
  SAPIEN_RENDER_CUDA_ORDINAL=0 CUDA_VISIBLE_DEVICES="${GPU}" "${CLIENT_PY}" scripts/eval/groot_n17_simpler_client.py \\
    ...
    --output "${OUT_ROOT}/results/\${name}.json" --episodes 24 --environment-seed 0 \\
    --max-episode-steps 300 --execution-horizon 4 --video-dir "${OUT_ROOT}/videos/\${name}" \\
    > "${OUT_ROOT}/logs/\${name}.log" 2>&1
done
"${CLIENT_PY}" scripts/eval/collect_groot_n17_decoder_action_token_medoid.py --out-root "${OUT_ROOT}" > "${OUT_ROOT}/logs/summary.log" 2>&1
EOF
chmod +x "${RUNNER}"
